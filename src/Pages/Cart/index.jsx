import React, { useState } from "react";
import { Link } from "react-router-dom";

import Menu from "@mui/material/Menu";
import MenuItem from "@mui/material/MenuItem";

import { RiShoppingBag4Line } from "react-icons/ri";
import { Button, Rating } from "@mui/material";
import CartItems from "./cartItems";

const CartPage = () => {



  return (
    <section className="section py-10 pb-10">
      <div className="container w-[80%] max-w-[80%] flex gap-5">
        <div className="leftPart w-[70%]">
          <div className="shadow-md rounded-md bg-white">
            <div className="py-2 px-3  border-b border-[rgba(0,0,0,0.1)]">
              <h2>Your Cart</h2>
              <p className="mt-0">
                There are <span className="font-bold text-primary"> 2 </span>{" "}
                products in your cart
              </p>
            </div>


            <CartItems size="9 ft." qty={1}/>
            <CartItems size="9 ft." qty={1}/>
            <CartItems size="9 ft." qty={1}/>
            <CartItems size="9 ft." qty={1}/>
            <CartItems size="9 ft." qty={1}/>
            <CartItems size="9 ft." qty={1}/>
            <CartItems size="9 ft." qty={1}/>
            
            
          </div>
        </div>

        <div className="rightPart w-[30%]">
          <div className="shadow-md rounded-md bg-white p-5">
            <h3 className="pb-3">Cart Totals</h3>
            <hr />

            <p className="flex items-center justify-between">
              <span className="text-[14px] font-[500]">Subtotal</span>
              <span className="text-primary font-bold">₹2599.00</span>
            </p>

            <p className="flex items-center justify-between">
              <span className="text-[14px] font-[500]">Shipping</span>
              <span className="text-primary font-bold">₹120.00</span>
            </p>

            <p className="flex items-center justify-between">
              <span className="text-[14px] font-[500]">Discount</span>
              <span className="text-[rgb(255,0,0)] font-bold">₹100.00</span>
            </p>

            <p className="flex items-center justify-between">
              <span className="text-[16px] font-[500]">Total</span>
              <span className="text-primary text-[16px] font-bold">
                ₹2700.00
              </span>
            </p>

            <br />

            <Button className="btn-org btn-lg w-full flex gap-4">
            {" "}
              <RiShoppingBag4Line className="text-[20px]" />
              Checkout
            </Button>
          </div>
        </div>
      </div>
    </section>
  );
};

export default CartPage;
